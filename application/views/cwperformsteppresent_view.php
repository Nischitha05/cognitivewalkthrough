...
<div id="forresultsview"></div>
          ...
          <input type="hidden" name="questionid1[]" id="questionsid" style='font-weight:bold;background-color:#F0FFFF' value="<?php echo $question; ?>"/>
          <input type="hidden" name="questionname1[]" id="questionsname" value="<?php echo $questionname; ?>"/>
              <input type="hidden" class="form-control" class='form-control' style='font-weight:bold;background-color:#F0FFFF' name="newquestion1[]"  id="new_questions<?php echo $question; ?>" value='<?php echo $newquestion ?>' readonly/>
...
<input type='hidden' name='nextstepimage' id='nextsteplistimage' value='<?php echo $row->image ?>' />
      ...
      <?php  $conn= mysqli_connect("localhost","root","","cognitivewalkthrough");
    		...
    		for ($i = 0; $i < count($questions_array); $i++) {
    			...
    			$newquestion = mysqli_real_escape_string($conn,$newquestion_array[$i]);
    		 echo "<input type='hidden' class='form-control' name='newquestion[]' style='font-weight:bold;background-color:#F0FFFF' id='new_questions$question' value='$newquestion' disabled/><br>";
        ...
        <div id='stepremove'>
        <?php
      foreach($qry7 as $each){
      ?><br><br>
       <h4>Question</h4>   <input type='text' name='questions[]' class='form-control' id='questions' style='font-weight:bold;background-color:#F0FFFF' value='<?php echo $each['questionname'] ?>' />
   <input type='hidden' name='questionid[]' id='questionsid' value='<?php echo $each['question'] ?>'/>
   <input type='hidden' name='questionname[]' id='questionsname' value='<?php echo $each['questionname']?>'/>
       <h4>Add question</h4>
       <input type='text' class='form-control' name='newquestion[]' style='font-weight:bold;background-color:#F0FFFF' id='new_questions<?php echo $each['stepname'] ?>' value='<?php echo $each['added_question']?>' readonly/>


       <h4>Answer:</h4>

       <?php if ($each['answer']=='no'){ ?>
       <select class="form-control" id='new_questions<?php echo $each['stepid'] ?>' name="answer[]" id="answer" >
  ...
  <option value="yes">yes</option></select>
       <!-- <input type='text' class='form-control' name='answer[]' style='font-weight:bold;width:100%;height:5%' id='new_questions<?php //echo $each['stepid'] ?>' value='<?php //echo $each['answer']?>' /><br> -->

<?php }else if ($each['answer']=='probably no'){ ?>
<select class="form-control" id='new_questions<?php echo $each['stepid'] ?>' name="answer[]" id="answer" >
<option value="">Select</option>
...
<?php }else if ($each['answer']=='unknown'){ ?>
<select class="form-control" id='new_questions<?php echo $each['stepid'] ?>' name="answer[]" id="answer" >
<option value="">Select</option>
<option value="no">no</option>
<option value="probably no">probably no</option>
...
<?php }else if ($each['answer']=='probably yes'){ ?>
<select class="form-control" id='new_questions<?php echo $each['stepid'] ?>' name="answer[]" id="answer" >
<option value="">Select</option>
<option value="no">no</option>
<option value="probably no">probably no</option>
...
<!-- <input type='text' class='form-control' name='answer[]' style='font-weight:bold;width:100%;height:5%' id='new_questions<?php //echo $each['stepid'] ?>' value='<?php //echo $each['answer']?>' /><br> -->
<?php } ?>
       <h4>Credible Story</h4><textarea name='question1_text[]' id='description<?php echo $each['question']?>' rows='3' cols='73' value='<?php echo $each['description']?>' ><?php echo $each['description']?></textarea><br>



<?php } ?>

 <br><br></div><div id='removenextaction'>
...
<b>Failure</b><input type='radio' name='rating' id='rating1' value='1' checked/>  <input type='radio' name='rating' id='rating2' value='2' />
...
//       formData.append('variantid',variantid);
    ...
    //   url: "<?= base_url() ?>index.php/variants/getaltstepimage1/",
    //    data: formData,
    //   processData: false,
  //   contentType: false,
       success: function(msg){
...
console.log(chosen_altmethod,"chosen_altmethod");
      ...
      $("#nextsteplist").remove();
        ...
        console.log(chosen_altmethod,"chosen_altmethod");
                    ...
                    $.ajax({
                        type: 'POST',
                       url: "<?= base_url() ?>index.php/variants/fetchprevstep/",
                        data: formData,
                        processData: false,
                contentType: false,
                        success: function (data) {
